...
class Counter:
    """
    代理录制，基于 mitmproxy 库拦截获取网络请求，自动抓取后生成
    将接口请求数据转换成 yaml 测试用例
    参考资料: https://blog.wolfogre.com/posts/usage-of-mitmproxy/
    """

    def __init__(self, filter_url: List, fliename: Text = './data/proxy_data.yaml'):
        self.num = 0
        self.file = fliename
        self.counter = 1
        # 过滤的URL
...
addons = [
    Counter(["https://zs-bjfyl-dy.danmu.hxzdm.com/douyin/webcast/comment_notify"])
]