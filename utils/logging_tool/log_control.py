...
class LogHandler:
    ...
    def __init__(
        ...
        self.log_path = ensure_path('\\logs\\log.log')
...
INFO = LogHandler(ensure_path(f"\\logs\\info-{now_time_day}.log"), level='info')
ERROR = LogHandler(ensure_path(f"\\logs\\error-{now_time_day}.log"), level='error')
WARNING = LogHandler(ensure_path(f'\\logs\\warning-{now_time_day}.log'))
