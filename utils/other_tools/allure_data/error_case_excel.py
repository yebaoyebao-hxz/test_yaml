...
class ErrorTestCase:
    """ 收集错误的excel """
    def __init__(self):
        self.test_case_path = ensure_path("./report/html/data/test-cases/")
    ...
    def get_headers(self, test_case):
        ...
        if test_case['testStage']['status'] == 'broken':
            ...
            path = ensure_path("\\report\\html\\data\\attachments\\" + _headers_attachment)
    ...
    def get_case_data(self, test_case):
        ...
        if test_case['testStage']['status'] == 'broken':
            ...
            path = ensure_path("\\report\\html\\data\\attachments\\" + _case_data_attachments)
            with open(path, 'r', encoding='utf-8') as file:
    ...
    def get_response(cls, test_case):
        ...
        if test_case['testStage']['status'] == 'broken':
            ...
            try:
                ...
                path = ensure_path("\\report\\html\\data\\attachments\\" + res_data_attachments)
                with open(path, 'r', encoding='utf-8') as file:
...
class ErrorCaseExcel:
    ...
    def __init__(self):
        _excel_template = ensure_path("./utils/other_tools/allure_data/自动化异常测试用例.xlsx")
        self._file_path = ensure_path("./Files/自动化异常测试用例.xlsx")
