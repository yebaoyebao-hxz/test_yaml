...
class TestCaseEnum(Enum):
    ...
    SLEEP = ("sleep", False)
    Ai_ASSERT = ("ai_assert", True)   # 是否启用AI断言
    AI_ASSERT_DETAIL = ("ai_assert_detail", True)  # AI断言的场景描述
    AI_ASSERT_PROMPT = ("ai_assert_prompt", False)  # 自定义prompt
...
class TestCase(BaseModel):
    ...
    ai_assert: Optional[Union[bool, Text]] = False
    ai_assert_detail: Optional[Union[bool, Text]] = None
    ai_assert_prompt: Optional[Union[bool, Text]] = None

    @field_validator('dependence_case_data', mode='before')
    @classmethod
    def coerce_dependent_cases(cls, v):
        ...
        if isinstance(v, list):
            ...
            for item in v:
                if isinstance(item, str):
                    result.append(DependentCaseData(case_id=item))
                elif isinstance(item, dict):
                    result.append(DependentCaseData(**item))
                else:
                    result.append(item)
            return result
        return v
