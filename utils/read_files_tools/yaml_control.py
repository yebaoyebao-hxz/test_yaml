...
# 处理 !=、== 等被误解析为 YAML tag 的值
class _TagSafeLoader(FullLoader):
    pass
def _tag_handler(loader, tag_suffix, node):
    if isinstance(node, yaml.nodes.ScalarNode):
        return tag_suffix
    return None
_TagSafeLoader.add_multi_constructor('', _tag_handler)


class GetYamlData:

    def __init__(self, file):
        self.file_dir = str(file)

    def get_yaml_data(self) -> dict:
        # 判断文件是否存在
        if os.path.exists(self.file_dir):
            data = open(self.file_dir, 'r', encoding='utf-8')
            res = yaml.load(data, Loader=_TagSafeLoader)
        else:
