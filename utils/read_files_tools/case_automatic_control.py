...
class TestCaseAutomaticGeneration:
    ...
    def case_date_path(self) -> Text:
        """返回 yaml 用例文件路径"""
        return ensure_path("\\data")

    @property
    def case_path(self) -> Text:
        """存放用例代码路径"""
        return ensure_path("\\test_case")
    ...
    def get_test_class_title(self):
        ...
        """
        # 提取文件名称
        _file_name = os.path.split(self.file_path)[1][:-3]
    ...
    def get_case_automatic(self) -> None:
        """ 自动生成 测试代码"""
        file_path = get_all_files(file_path=ensure_path("\\data"), yaml_data_switch=True)
        # 测试调用
        # target_yaml_file = r"E:\yebao\test_yaml\data\登录接口测试用例_共6条.yaml"
        for file in file_path:
            ...
            if 'proxy_data.yaml' not in file:
                ...
                # 预填充缓存：CaseData.case_process 读取 YAML → CacheHandler.update_cache
                # 确保 GetTestCase.case_data() 在 import 时能直接从内存缓存读到数据
                try:
                    case_data_list = CaseData(file).case_process(case_id_switch=True)
                    for item in case_data_list:
                        for case_id, case_data in item.items():
